...
public class FacultyServiceImpl implements FacultyService {
    ...
    private FacultyResponse toResponse(FacultyModel faculty) {
        List<UniversityResponse> universities = List.of();

        if (faculty.getUniversity() != null) {

            UniversityResponse uniResponse =
                    UniversityResponse.builder()
                            .id(faculty.getUniversity().getId())
                            .name(faculty.getUniversity().getName())
                            .country(faculty.getUniversity().getCountry())
                            .city(faculty.getUniversity().getCity())
                            .description(faculty.getUniversity().getDescription())
                            .build();

            universities = List.of(uniResponse);
        }
